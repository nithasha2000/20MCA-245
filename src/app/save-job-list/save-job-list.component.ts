...
@Component({
  selector: 'app-save-job-list',
  templateUrl: './save-job-list.component.html',
  styleUrls: ['./save-job-list.component.css']
})
export class SaveJobListComponent implements OnInit{
  ...
  ngOnInit() {
    ...
    if (this.userData) {
      ...
      this.http.post('http://127.0.0.1:8000/save-job-list/', payload).subscribe((response: any) => {
      try {
        if (response.message === 'success') {
          this.job_list = response.data
        } 
  ...
  onUnSave(job: any)
  {
    const payload = {
      "username": this.userData.username,
      "role": this.userData.role,
      "save_post": job.save_post
    };

    this.http.post('http://127.0.0.1:8000/unsave-job/', payload).subscribe((response: any) => {
      try {
        if (response.message === 'success') {
          ...
          if (Array.isArray(response.data)) {
            response.data.forEach((item: any) => {
              this.toastr.error(item, 'Job UnSave Failed', {
                  positionClass: 'toast-top-center',
                });
            });
          } else {
              this.toastr.error(response.data, 'Job UnSave Failed', {
                positionClass: 'toast-top-center',
              });
          }
        }
      } catch (error) {
        this.toastr.error('Job UnSave Failed', 'Try Again',{
            positionClass: 'toast-top-center',
          });
        }
  });
